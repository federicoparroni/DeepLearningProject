...
\textit{validation accuracy }= 0.915 with \textit{threshold}=0.5
\\
...
Since the model has been developed for security reasons a possible idea is to tolerate at most a 0.001\% of false positive to achieve that the treshold has to be set to 0.975 as suggested by the ROC curve, so that the network will recognize a couple of faces $(i,j)$ as the same person only when $Y(i,j)>0.975$. The performance between two different tresholds can be compared using the confusion matrices taking into account also the
...
The accuracies obtained changing the two tresholds are comparable but the $precision$ is considerably higher when the treshold is set to 0.975.
